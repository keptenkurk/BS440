...
class Plugin:
	...
	def get_id(self, iniid, text, type, options=""):
		global user
		global personsection
		global pluginconfig
		global log
		try:
			rid = pluginconfig.get(personsection, iniid)
			if not self.exists_id(rid):
				raise Exception
		except:
	...
	def get_realid(self, iniid,default):
		...
		global pluginconfig
		try:
			return pluginconfig.get(personsection, iniid)
		except:
			write_config = True
	...
	def execute(self, globalconfig, persondata, weightdata, bodydata):
		...
		url_mass = 'http://%s/json.htm?type=command&param=udevice&hid=%s&' \
				  'idx=%s&dunit=%s&dtype=93&dsubtype=1&nvalue=0&svalue=%s'
		...
		try:
			...
			# Mass
			weightid = self.get_id('weight_id', 'Weight', SensorScale)
			fatmassid = self.get_id('fat_mass_id', 'Fat Mass', SensorScale)
			watermassid = self.get_id('watermass_id', 'Water Mass', SensorScale)
			musclemassid = self.get_id('muscle_mass_id', 'Muscle Mass', SensorScale)
			bonemassid = self.get_id('bone_mass_id', 'Bone Mass', SensorScale)
			lbmid = self.get_id('lbm_id', 'Lean Body Mass', SensorScale)
		...
		if write_config:
			fconfigfile = open(configfile, 'wb')
			pluginconfig.write(fconfigfile)
			fconfigfile.close()
			write_config = False
			"""
			with open(configfile, 'wb') as fconfigfile:
				fpluginconfig.write(configfile)
				configfile.close()
				write_config = False
			"""
